function(input=import 'defaultInput.libsonnet')
  ...
  [
    ...
    secret.externalSecretExtract('prometheus-secret', config.namespace),
    k8s.db.database(
      name=name,
      namespace=config.namespace
    ),
    ...
    k8s.secret.externalSecretExtract('oidc-grafana', config.namespace, key='oidc/grafana'),
      ...
      values={
        ...
        grafana: {
          ...
          'grafana.ini': {
            ...
            server: {
              root_url: 'https://grafana.%s' % input.globals.config.domain,
            },
            smtp: {
              enabled: true,
              host: 'mail.%s:465' % input.globals.config.domain,
              from_address: 'grafana@%s' % input.globals.config.domain,
            },
            'auth.generic_oauth': {
              name: 'authelia',
              enabled: true,
              ...
              auth_url: '%s/api/oidc/authorization' % input.globals.config.urls.auth,
              token_url: '%s/api/oidc/token' % input.globals.config.urls.auth,
              api_url: '%s/api/oidc/userinfo' % input.globals.config.urls.auth,
          ...
          ingress: {
            ...
            hosts: [
              'grafana.%s' % input.globals.config.domain,
            ],
            tls: [
              {
                hosts: [
