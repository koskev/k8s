{
  local outerSelf = self,
  '#new':: { 'function': {
    help:
      |||
        A Service is an abstraction which defines a logical set of pods and a policy by which to access them - sometimes called a micro-service.
      |||,
  } },
  new(terraformName):: self.functions(terraformName) + {
    _type:: 'tf',
    ref():: outerSelf.ref(terraformName),
    resource+: {
      kubernetes_service+: { [terraformName]+: {
      } },
    },
  }
  ,
  functions(terraformName):: {
    withForEach(value):: self { resource+: {
      kubernetes_service+: { [terraformName]+: { for_each: value } },
    } },
    withDependsOn(value):: self { resource+: {
      kubernetes_service+: { [terraformName]+: { depends_on: value } },
    } },
    withCount(value):: self { resource+: {
      kubernetes_service+: { [terraformName]+: { count: value } },
    } },
    withLifecycle(value):: self { resource+: {
      kubernetes_service+: { [terraformName]+: { lifecycle: value } },
    } },
    withProvider(value):: self { resource+: {
      kubernetes_service+: { [terraformName]+: { provider: value } },
    } },
    withProviders(value):: self { resource+: {
      kubernetes_service+: { [terraformName]+: { providers: value } },
    } },
    addCustomData(name, value):: self { resource+: {
      kubernetes_service+: { [terraformName]+: { [name]: value } },
    } },
    withId(value):: self { resource+: {
      kubernetes_service+: { [terraformName]+: { id: value } },
    } },
    '#withWaitForLoadBalancer':: { 'function': {
      help:
        |||
          Terraform will wait for the load balancer to have at least 1 endpoint before considering the resource created.
        |||,
    } },
    withWaitForLoadBalancer(value):: self { resource+: {
      kubernetes_service+: { [terraformName]+: { wait_for_load_balancer: value } },
    } },

  }
  ,
  ref(terraformName):: {
    local refSelf = self,
    plain(suffix=''):: '${ kubernetes_service.%s%s }' % [terraformName, suffix],
    fields:: {
      id(suffix=''):: refSelf.plain('.id%s' % suffix),
      status(suffix=''):: refSelf.plain('.status%s' % suffix),
      '#wait_for_load_balancer':: { 'function': {
        help:
          |||
            Terraform will wait for the load balancer to have at least 1 endpoint before considering the resource created.
          |||,
      } },
      wait_for_load_balancer(suffix=''):: refSelf.plain('.wait_for_load_balancer%s' % suffix),

    },

  },

}
