{
  ...
  functions(terraformName):: {
    withForEach(value):: self { resource+: {
      vault_pki_secret_backend_config_auto_tidy+: { [terraformName]+: { for_each: value } },
    } },
    withDependsOn(value):: self { resource+: {
      vault_pki_secret_backend_config_auto_tidy+: { [terraformName]+: { depends_on: value } },
    } },
    withCount(value):: self { resource+: {
      vault_pki_secret_backend_config_auto_tidy+: { [terraformName]+: { count: value } },
    } },
    withLifecycle(value):: self { resource+: {
      vault_pki_secret_backend_config_auto_tidy+: { [terraformName]+: { lifecycle: value } },
    } },
    withProvider(value):: self { resource+: {
      vault_pki_secret_backend_config_auto_tidy+: { [terraformName]+: { provider: value } },
    } },
    withProviders(value):: self { resource+: {
      vault_pki_secret_backend_config_auto_tidy+: { [terraformName]+: { providers: value } },
    } },
    addCustomData(name, value):: self { resource+: {
      vault_pki_secret_backend_config_auto_tidy+: { [terraformName]+: { [name]: value } },
    } },
    '#withAcmeAccountSafetyBuffer':: { 'function': {
      help:
        |||
          The amount of time that must pass after creation that an account with no orders is marked revoked, and the amount of time after being marked revoked or deactivated.
        |||,
    } },
    withAcmeAccountSafetyBuffer(value):: self { resource+: {
    ...
    withTidyRevokedCerts(value):: self { resource+: {
  ...
  ref(terraformName):: {
    ...
    fields:: {
      '#acme_account_safety_buffer':: { 'function': {
        help:
          |||
            The amount of time that must pass after creation that an account with no orders is marked revoked, and the amount of time after being marked revoked or deactivated.
          |||,
      ...
      '#maintain_stored_certificate_counts':: { 'function': {
        help:
          |||
            This configures whether stored certificate are counted upon initialization of the backend, and whether during normal operation, a running count of certificates stored is maintained.
          |||,
      ...
      '#max_startup_backoff_duration':: { 'function': {
        help:
          |||
            The maximum amount of time auto-tidy will be delayed after startup.
          |||,
      ...
      '#min_startup_backoff_duration':: { 'function': {
        help:
          |||
            The minimum amount of time auto-tidy will be delayed after startup.
          |||,
      ...
      '#publish_stored_certificate_count_metrics':: { 'function': {
        help:
          |||
            This configures whether the stored certificate count is published to the metrics consumer.
          |||,
      ...
      '#revocation_queue_safety_buffer':: { 'function': {
        help:
          |||
            The amount of time that must pass from the cross-cluster revocation request being initiated to when it will be slated for removal.
          |||,
      ...
      '#tidy_cmpv2_nonce_store':: { 'function': {
        help:
          |||
            Set to true to enable tidying up the CMPv2 nonce store.
          |||,
      ...
      '#tidy_cross_cluster_revoked_certs':: { 'function': {
        help:
          |||
            Set to true to enable tidying up the cross-cluster revoked certificate store.
          |||,
      ...
      '#tidy_move_legacy_ca_bundle':: { 'function': {
        help:
          |||
            Set to true to move the legacy ca_bundle from /config/ca_bundle to /config/ca_bundle.bak.
          |||,
      ...
      '#tidy_revocation_queue':: { 'function': {
        help:
          |||
            Set to true to remove stale revocation queue entries that haven't been confirmed by any active cluster.
          |||,
      ...
      '#tidy_revoked_cert_issuer_associations':: { 'function': {
        help:
          |||
            Set to true to validate issuer associations on revocation entries. This helps increase the performance of CRL building and OCSP responses.
          |||,
