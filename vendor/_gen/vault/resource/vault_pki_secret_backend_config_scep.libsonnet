{
  ...
  functions(terraformName):: {
    ...
    addCustomData(name, value):: self { resource+: {
      vault_pki_secret_backend_config_scep+: { [terraformName]+: { [name]: value } },
    } },
    '#withAllowedDigestAlgorithms':: { 'function': {
      help:
        |||
          List of allowed digest algorithms for SCEP requests
        |||,
    } },
    withAllowedDigestAlgorithms(value):: self { resource+: {
      vault_pki_secret_backend_config_scep+: { [terraformName]+: { allowed_digest_algorithms: value } },
    } },
    '#withAllowedEncryptionAlgorithms':: { 'function': {
      help:
        |||
          List of allowed encryption algorithms for SCEP requests
        |||,
    } },
    withAllowedEncryptionAlgorithms(value):: self { resource+: {
    ...
    withLogLevel(value):: self { resource+: {
    ...
    '#withRestrictCaChainToIssuer':: { 'function': {
      help:
        |||
          If true, only return the issuer CA, otherwise the entire CA certificate chain will be returned if available from the PKI mount
        |||,
    } },
    withRestrictCaChainToIssuer(value):: self { resource+: {
  ...
  ref(terraformName):: {
    ...
    fields:: {
      '#allowed_digest_algorithms':: { 'function': {
        help:
          |||
            List of allowed digest algorithms for SCEP requests
          |||,
      ...
      '#allowed_encryption_algorithms':: { 'function': {
        help:
          |||
            List of allowed encryption algorithms for SCEP requests
          |||,
      ...
      '#restrict_ca_chain_to_issuer':: { 'function': {
        help:
          |||
            If true, only return the issuer CA, otherwise the entire CA certificate chain will be returned if available from the PKI mount
          |||,
