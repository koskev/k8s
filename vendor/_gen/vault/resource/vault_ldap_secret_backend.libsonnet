{
  ...
  functions(terraformName):: {
    withForEach(value):: self { resource+: {
      vault_ldap_secret_backend+: { [terraformName]+: { for_each: value } },
    } },
    withDependsOn(value):: self { resource+: {
      vault_ldap_secret_backend+: { [terraformName]+: { depends_on: value } },
    } },
    withCount(value):: self { resource+: {
      vault_ldap_secret_backend+: { [terraformName]+: { count: value } },
    } },
    withLifecycle(value):: self { resource+: {
      vault_ldap_secret_backend+: { [terraformName]+: { lifecycle: value } },
    } },
    withProvider(value):: self { resource+: {
      vault_ldap_secret_backend+: { [terraformName]+: { provider: value } },
    } },
    withProviders(value):: self { resource+: {
      vault_ldap_secret_backend+: { [terraformName]+: { providers: value } },
    } },
    addCustomData(name, value):: self { resource+: {
      vault_ldap_secret_backend+: { [terraformName]+: { [name]: value } },
    } },
    ...
    '#withClientTlsCert':: { 'function': {
      help:
        |||
          Client certificate to provide to the LDAP server, must be x509 PEM encoded.
        |||,
    } },
    withClientTlsCert(value):: self { resource+: {
      vault_ldap_secret_backend+: { [terraformName]+: { client_tls_cert: value } },
    } },
    '#withClientTlsKey':: { 'function': {
      help:
        |||
          Client certificate key to provide to the LDAP server, must be x509 PEM encoded.
        |||,
    } },
    withClientTlsKey(value):: self { resource+: {
      vault_ldap_secret_backend+: { [terraformName]+: { client_tls_key: value } },
    } },
    '#withConnectionTimeout':: { 'function': {
      help:
        |||
          Timeout, in seconds, when attempting to connect to the LDAP server before trying the next URL in the configuration.
        |||,
    } },
    withConnectionTimeout(value):: self { resource+: {
    ...
    '#withInsecureTls':: { 'function': {
      help:
        |||
          Skip LDAP server SSL Certificate verification - insecure and not recommended for production use.
        |||,
    } },
    withInsecureTls(value):: self { resource+: {
    ...
    '#withRequestTimeout':: { 'function': {
      help:
        |||
          Timeout, in seconds, for the connection when making requests against the server before returning back an error.
        |||,
    } },
    withRequestTimeout(value):: self { resource+: {
    ...
    withSchema(value):: self { resource+: {
    ...
    withUrl(value):: self { resource+: {
      vault_ldap_secret_backend+: { [terraformName]+: { url: value } },
    } },
    '#withUserattr':: { 'function': {
      help:
        |||
          Attribute used for users (default: cn)
        |||,
    } },
    withUserattr(value):: self { resource+: {
    ...
    withUserdn(value):: self { resource+: {
  ...
  ref(terraformName):: {
    ...
    fields:: {
      ...
      '#skip_static_role_import_rotation':: { 'function': {
        help:
          |||
            Skip rotation of static role secrets on import.
          |||,
