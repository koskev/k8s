{
  ...
  functions(terraformName):: {
    ...
    addCustomData(name, value):: self { resource+: {
      vault_okta_auth_backend+: { [terraformName]+: { [name]: value } },
    } },
    ...
    withApiTokenWo(value):: self { resource+: {
      vault_okta_auth_backend+: { [terraformName]+: { api_token_wo: value } },
    } },
    '#withApiTokenWoVersion':: { 'function': {
      help:
        |||
          Version counter for write-only api_token.
        |||,
    } },
    withApiTokenWoVersion(value):: self { resource+: {
    ...
    '#withBypassOktaMfa':: { 'function': {
      help:
        |||
          When true, requests by Okta for a MFA check will be bypassed. This also disallows certain status checks on the account, such as whether the password is expired.
        |||,
    } },
    withBypassOktaMfa(value):: self { resource+: {
