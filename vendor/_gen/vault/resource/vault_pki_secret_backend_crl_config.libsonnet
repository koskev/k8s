{
  ...
  functions(terraformName):: {
    withForEach(value):: self { resource+: {
      vault_pki_secret_backend_crl_config+: { [terraformName]+: { for_each: value } },
    } },
    withDependsOn(value):: self { resource+: {
      vault_pki_secret_backend_crl_config+: { [terraformName]+: { depends_on: value } },
    } },
    withCount(value):: self { resource+: {
      vault_pki_secret_backend_crl_config+: { [terraformName]+: { count: value } },
    } },
    withLifecycle(value):: self { resource+: {
      vault_pki_secret_backend_crl_config+: { [terraformName]+: { lifecycle: value } },
    } },
    withProvider(value):: self { resource+: {
      vault_pki_secret_backend_crl_config+: { [terraformName]+: { provider: value } },
    } },
    withProviders(value):: self { resource+: {
      vault_pki_secret_backend_crl_config+: { [terraformName]+: { providers: value } },
    } },
    addCustomData(name, value):: self { resource+: {
      vault_pki_secret_backend_crl_config+: { [terraformName]+: { [name]: value } },
    } },
    '#withAutoRebuild':: { 'function': {
      help:
        |||
          Enables or disables periodic rebuilding of the CRL upon expiry.
        |||,
    } },
    withAutoRebuild(value):: self { resource+: {
      vault_pki_secret_backend_crl_config+: { [terraformName]+: { auto_rebuild: value } },
    } },
    '#withAutoRebuildGracePeriod':: { 'function': {
      help:
        |||
          Grace period before CRL expiry to attempt rebuild of CRL.
        |||,
    } },
    withAutoRebuildGracePeriod(value):: self { resource+: {
    ...
    withExpiry(value):: self { resource+: {
    ...
    withMaxCrlEntries(value):: self { resource+: {
  ...
  ref(terraformName):: {
    ...
    fields:: {
      ...
      '#auto_rebuild_grace_period':: { 'function': {
        help:
          |||
            Grace period before CRL expiry to attempt rebuild of CRL.
          |||,
      ...
      '#cross_cluster_revocation':: { 'function': {
        help:
          |||
            Enable cross-cluster revocation request queues.
          |||,
      ...
      '#delta_rebuild_interval':: { 'function': {
        help:
          |||
            Interval to check for new revocations on, to regenerate the delta CRL.
          |||,
      ...
      '#unified_crl_on_existing_paths':: { 'function': {
        help:
          |||
            Enables serving the unified CRL and OCSP on the existing, previously cluster-local paths.
          |||,
