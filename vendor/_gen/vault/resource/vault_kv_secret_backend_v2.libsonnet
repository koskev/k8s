{
  ...
  functions(terraformName):: {
    ...
    addCustomData(name, value):: self { resource+: {
      vault_kv_secret_backend_v2+: { [terraformName]+: { [name]: value } },
    } },
    '#withCasRequired':: { 'function': {
      help:
        |||
          If true, all keys will require the cas parameter to be set on all write requests.
        |||,
    } },
    withCasRequired(value):: self { resource+: {
      vault_kv_secret_backend_v2+: { [terraformName]+: { cas_required: value } },
    } },
    '#withDeleteVersionAfter':: { 'function': {
      help:
        |||
          If set, specifies the length of time before a version is deleted
        |||,
    } },
    withDeleteVersionAfter(value):: self { resource+: {
