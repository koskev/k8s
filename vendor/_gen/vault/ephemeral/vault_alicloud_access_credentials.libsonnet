{
  ...
  new(terraformName, mount, role):: self.functions(terraformName) + {
    ...
    ephemeral+: {
      vault_alicloud_access_credentials+: { [terraformName]+: {
        mount: mount,
        role: role,
      } },
    },
  ...
  functions(terraformName):: {
    withForEach(value):: self { ephemeral+: {
      vault_alicloud_access_credentials+: { [terraformName]+: { for_each: value } },
    } },
    withDependsOn(value):: self { ephemeral+: {
      vault_alicloud_access_credentials+: { [terraformName]+: { depends_on: value } },
    } },
    withCount(value):: self { ephemeral+: {
      vault_alicloud_access_credentials+: { [terraformName]+: { count: value } },
    } },
    withLifecycle(value):: self { ephemeral+: {
      vault_alicloud_access_credentials+: { [terraformName]+: { lifecycle: value } },
    } },
    withProvider(value):: self { ephemeral+: {
      vault_alicloud_access_credentials+: { [terraformName]+: { provider: value } },
    } },
    withProviders(value):: self { ephemeral+: {
      vault_alicloud_access_credentials+: { [terraformName]+: { providers: value } },
    } },
    addCustomData(name, value):: self { ephemeral+: {
      vault_alicloud_access_credentials+: { [terraformName]+: { [name]: value } },
    } },
    '#withMount':: { 'function': {
      help:
        |||
