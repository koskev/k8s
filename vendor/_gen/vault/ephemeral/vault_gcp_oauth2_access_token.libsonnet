{
  ...
  new(terraformName, mount):: self.functions(terraformName) + {
    ...
    ephemeral+: {
      vault_gcp_oauth2_access_token+: { [terraformName]+: {
        mount: mount,
      } },
    },
  }
  ...
  functions(terraformName):: {
    ...
    addCustomData(name, value):: self { ephemeral+: {
      vault_gcp_oauth2_access_token+: { [terraformName]+: { [name]: value } },
    } },
    '#withImpersonatedAccount':: { 'function': {
      help:
        |||
          GCP Secret Impersonated Account to generate OAuth2 access token for. Mutually exclusive with `roleset` and `static_account`.
        |||,
    } },
    withImpersonatedAccount(value):: self { ephemeral+: {
    ...
    withMaxRetries(value):: self { ephemeral+: {
