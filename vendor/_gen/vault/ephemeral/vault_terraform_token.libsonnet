{
  ...
  new(terraformName, role_name):: self.functions(terraformName) + {
    ...
    ephemeral+: {
      vault_terraform_token+: { [terraformName]+: {
        role_name: role_name,
      } },
    },
  }
  ...
  functions(terraformName):: {
    withForEach(value):: self { ephemeral+: {
      vault_terraform_token+: { [terraformName]+: { for_each: value } },
    } },
    withDependsOn(value):: self { ephemeral+: {
      vault_terraform_token+: { [terraformName]+: { depends_on: value } },
    } },
    withCount(value):: self { ephemeral+: {
      vault_terraform_token+: { [terraformName]+: { count: value } },
    } },
    withLifecycle(value):: self { ephemeral+: {
      vault_terraform_token+: { [terraformName]+: { lifecycle: value } },
    } },
    withProvider(value):: self { ephemeral+: {
      vault_terraform_token+: { [terraformName]+: { provider: value } },
    } },
    withProviders(value):: self { ephemeral+: {
      vault_terraform_token+: { [terraformName]+: { providers: value } },
    } },
    addCustomData(name, value):: self { ephemeral+: {
      vault_terraform_token+: { [terraformName]+: { [name]: value } },
    } },
    '#withMount':: { 'function': {
      help:
        |||
