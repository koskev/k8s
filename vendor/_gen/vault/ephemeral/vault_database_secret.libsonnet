{
  ...
  new(terraformName, mount, name):: self.functions(terraformName) + {
    ...
    ephemeral+: {
      vault_database_secret+: { [terraformName]+: {
        mount: mount,
        name: name,
      } },
    },
  ...
  functions(terraformName):: {
    withForEach(value):: self { ephemeral+: {
      vault_database_secret+: { [terraformName]+: { for_each: value } },
    } },
    withDependsOn(value):: self { ephemeral+: {
      vault_database_secret+: { [terraformName]+: { depends_on: value } },
    } },
    withCount(value):: self { ephemeral+: {
      vault_database_secret+: { [terraformName]+: { count: value } },
    } },
    withLifecycle(value):: self { ephemeral+: {
      vault_database_secret+: { [terraformName]+: { lifecycle: value } },
    } },
    withProvider(value):: self { ephemeral+: {
      vault_database_secret+: { [terraformName]+: { provider: value } },
    } },
    withProviders(value):: self { ephemeral+: {
      vault_database_secret+: { [terraformName]+: { providers: value } },
    } },
